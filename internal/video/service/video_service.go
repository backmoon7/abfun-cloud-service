...
func (s *VideoService) GetFeed(limit, offset int, tagID uint) ([]model.Video, error) {
        videos, err := s.repo.GetVideos(limit, offset, tagID)
        if err != nil {
                return nil, err
        }
        
        for i := range videos {
                if tagIDs, err := s.repo.GetVideoTagIDs(videos[i].ID); err == nil {
                        videos[i].TagIDs = tagIDs
                }
        }
...
func (s *VideoService) SearchVideos(query string) ([]model.Video, error) {
	...
	for _, hit := range hits {
		...
		v.ID = uint(h["id"].(float64))
                if tagIDs, err := s.repo.GetVideoTagIDs(v.ID); err == nil {
                        v.TagIDs = tagIDs
                }
...
func (s *VideoService) GetVideoByID(videoID uint) (*model.Video, error) {
video, tagIDs, err := s.repo.FindByID(videoID)
if err != nil {
return nil, err
}
